...
if(isset($_SESSION['delete_msg'])){
    echo '<div class="position-fixed w-100" style="top: 0; left: 0; z-index: 9999;">';
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert">';
    echo '<strong>Success!</strong> ' . $_SESSION['delete_msg'];
    echo '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
    echo '</div>';
    echo '</div>';
    // Unset the session variable to remove the message
...
<div class="sidebar pe-4 pb-3">
    <nav class="navbar bg-light navbar-light">
        ...
        <div class="navbar-nav w-100">
            ...
            <a href="security.php" class="nav-item nav-link"><i class="fa fa-shield-alt me-2"></i>Security</a>
        </div>
    </nav>
</div>
        <!-- Sidebar End -->
    ...
    $sql = "SELECT user.id as uid, account.id as aid, card.id, account.acc_number, card.exp_date, card.balance, card.acc_limit
            FROM card
            JOIN user ON card.user_id = user.id
            JOIN account ON card.acc_id = account.id";
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>
                    ...
                    <td>" . $row["balance"] . "</td>
                    <td>" . $row["acc_limit"] . "</td>
                    <td><a href='../update&delete/update_card.php?id=" . $row["id"] . "' class='btn btn-success'>Update</a></td>
                    <td><a href='../update&delete/delete_card.php?id=" . $row["id"] . "'' class='btn btn-danger'>Delete</a></td>
                </tr>";
        }
    } else {
