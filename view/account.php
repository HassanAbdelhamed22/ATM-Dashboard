...
<body>
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Last_Tran_Date</th>
    ...
    $sql = "SELECT account.id as acc_id, user.id as user_id, user.name, account.acc_number, account.balance, account.last_tran_date
            FROM account
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr>
                    ...
                    <td>" . $row["balance"] . "</td>
                    <td>" . $row["last_tran_date"] . "</td>
                    <td><a href='../update&delete/update_acc.php?id=" . $row["acc_id"] . "' class='btn btn-success'>Update</a></td>
                    <td><a href='../update&delete/delete_acc.php?id=" . $row["acc_id"] . "'' class='btn btn-danger'>Delete</a></td>
                </tr>";
        }
    } else {
