...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT * FROM atm_admin WHERE id = $id";
...
<?php
    ...
    do {
        if (empty($id) || empty($name) || empty($email) || empty($uname) || empty($password)) {
            $errorMessage = 'All the fields are required';
            break;
        }
        $sql = "UPDATE atm_admin SET name = '$name', email = '$email', uname = '$uname', password = '$password' WHERE id = '$id'";
        $result = $dbController->connection->query($sql);
        if (!$result) {
