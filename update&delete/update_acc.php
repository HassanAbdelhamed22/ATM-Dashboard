...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $id = $_GET['id'];
    $sql = "SELECT acc_number , balance , last_tran_date FROM account WHERE id = ?";
    ...
    $balance = $row['balance'];
    $last_tran_date = $row['last_tran_date'];
?>
...
<form method="post">
    ...
    <div class="row g-3">
        <div class="col-md-6">
            ...
            <input type="text" name="acc_number" class="form-control" id="acc_number" value="<?php echo $acc_number; ?>">
        </div>
        <div class="col-md-6">
            <label for="phone" class="form-label">Balance</label>
            <input type="text" name="balance" class="form-control" id="balance" value="<?php echo $balance; ?>">
        </div>
        <div class="col-md-6">
            <label for="last_tran_date" class="form-label">Last_Tran_Date</label>
            <input type="text" name="last_tran_date" class="form-control" id="last_tran_date" value="<?php echo $last_tran_date; ?>">
        </div>
...
<?php 
    ...
    $last_tran_date = $_POST['last_tran_date'];
    do {
        if (empty($id) || empty($acc_number) || empty($balance) || empty($last_tran_date) ) {
            $errorMessage = 'All the fields are required';
            break;
        }
        $sql = "UPDATE account SET acc_number = ?, balance = ?, last_tran_date = ? WHERE id = ?";
        $stmt = $dbController->connection->prepare($sql);
        $stmt->bind_param("sssi", $acc_number, $balance, $last_tran_date, $id);
        $stmt->execute();
        if ($stmt->error) {
        ...
        echo '<script>window.location.href = "../view/account.php";</script>';
    } while (false);
} 
?>
