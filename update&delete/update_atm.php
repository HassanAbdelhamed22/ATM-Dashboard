...
if(isset($_GET['id'])){
    ...
    $result = mysqli_query($dbController->connection, $sql);
    if(!$result){
...
if(isset($_POST['update'])){
    ...
    $av_cash = $_POST['av_cash'];
    $type = $_POST['type'];
    $status = $_POST['status'];

    $query = "update atm set location = '$location' , av_cash = '$av_cash' 
    , type = '$type', status = '$status'  where id = '$idnew'";
    ...
    if(!$result){
        ...
        $_SESSION['update_msg'] = "You have successfully updated the data";
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT id, location, av_cash, type ,status FROM atm WHERE id = $id";
...
?>
    ...
    <form method="post">
        ...
        <div class="col-md-6">
            <label for="av_cash" class="form-label">Available Cash</label>
            <input type="text" name="av_cash" class="form-control" id="av_cash"value="<?php echo $row['av_cash'] ?>">
        </div>
        <div class="col-md-6">
            <label for="type" class="form-label">Type</label>
            <input type="text" name="type" class="form-control" id="type"value="<?php echo $row['type'] ?>">
        </div>
        <div class="col-md-6">
...
<?php
    ...
    if (empty($id) || empty($location) || empty($av_cash) || empty($type)) {
        $errorMessage = 'All fields are required';
    } else {
        // Prepare the SQL statement with placeholders
        $sql = "UPDATE atm SET location = ?, av_cash = ?, type = ? , status = ? WHERE id = ?";

        // Prepare the statement
        $stmt = $dbController->connection->prepare($sql);

        // Bind the parameters to the statement
        $stmt->bind_param('sssi', $location, $av_cash, $type, $id, $status);
        ...
        if (!$result) {
            ...
            $successMessage = 'Atm updated successfully';
            // Redirect to the card.php page after a successful update
            echo '<script>window.location.href = "../view/atm.php";</script>';
        }
    }
}
?>
